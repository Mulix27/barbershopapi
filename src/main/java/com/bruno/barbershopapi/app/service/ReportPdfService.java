...
public class ReportPdfService {
    ...
    public String generateBase64(FullReportResponse report, String shopName, boolean singleBarber) throws Exception {
        ...
        buildHeader(doc, shopName, report.periodLabel(), report.from(), report.to(), singleBarber);
        addRule(doc, GOLD);
        doc.add(Chunk.NEWLINE);

        buildMetricCards(doc, report, singleBarber);
        doc.add(Chunk.NEWLINE);

        if (report.topServices() != null && !report.topServices().isEmpty()) {
            addSectionTitle(doc, "Servicios más Rentables");
            buildServicesTable(doc, report, singleBarber);
            doc.add(Chunk.NEWLINE);
        }

        if (!singleBarber && report.barberPerformance() != null && !report.barberPerformance().isEmpty()) {
    ...
    private void buildHeader(
        ...
        String titleText = singleBarber ? "Insights Personales" : "Informe de Negocios";

        Paragraph title = new Paragraph(titleText, F_TITLE);
    ...
    private void buildMetricCards(Document doc, FullReportResponse r, boolean singleBarber) throws DocumentException {
        ...
        double revenue = summary != null && summary.totalRevenue() != null
                ? summary.totalRevenue().doubleValue()
                : 0;

        double avgTicket = summary != null && summary.averageTicket() != null
                ? summary.averageTicket().doubleValue()
                : 0;

        long sales = summary != null && summary.totalSales() != null
                ? summary.totalSales()
                : 0;
        ...
        double netProfit = singleBarber ? revenue : revenue * 0.41;
        String netProfitLabel = singleBarber ? "Ganancia Neta Real" : "Ganancia Neta (41%)";

        addMetricCard(t, fmt(revenue), "Ingresos Totales");
        addMetricCard(t, fmt(netProfit), netProfitLabel);
    ...
    private void buildServicesTable(Document doc, FullReportResponse r, boolean singleBarber) throws DocumentException {
        ...
        for (var svc : r.topServices()) {
            ...
            double net = singleBarber ? rev : rev * 0.82;
        ...
        double totalNet = singleBarber ? totalRev : totalRev * 0.82;

        addTotalRow(t, "Total", "", "", fmt(totalNet));
    ...
    private void buildAppointmentTable(Document doc, FullReportResponse r) throws DocumentException {
        ...
        String occupancyRate = am.totalAppointments() > 0
